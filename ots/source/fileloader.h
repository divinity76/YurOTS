...
public:
    ...
    void endNode();
    int32_t setProps(void* data, unsigned size);
...
public:
    inline bool writeData(const void* data, int32_t size, bool unescape)
    {
        ...
        {
            unsigned char c = *(((uint8_t*)data) + i);
            if(unescape && (c == NODE_START || c == NODE_END || c == ESCAPE_CHAR))
            {
                unsigned char tmp = ESCAPE_CHAR;
#ifdef __USE_ZLIB__
...
public:
    ...
    inline bool getByte(unsigned char& ret)
    {
        return getType(ret);
    }
...
public:
    ...
    inline void addByte(unsigned char ret)
    {
        addType(ret);
    }
