...
uint32_t Monster::getRandom()
{
    ...
    return (uint32_t)std::ceil((double)random_range(0, CHANCE_MAX) / g_config.DROP_RATE);
}
...
Creature* Monster::findTarget(int32_t range, bool &canReach, const Creature *ignoreCreature /*= NULL*/)
{
    ...
    if(targetlist.empty())
    {
        ...
        {
            size_t index = random_range(0, (int32_t)unreachablelist.size() - 1);
            canReach = false;
    ...
    size_t index = random_range(0, (int32_t)targetlist.size() - 1);
    canReach = true;
    return targetlist[index];
}

int32_t Monster::onThink(int32_t& newThinkTicks)
{
    bool yelled = false;
    ...
    if(state == STATE_TARGETNOTREACHABLE || attackedCreature != 0)
    {
        if(delay > 0)
        {
            newThinkTicks = (int32_t)delay;
            int32_t ret = oldThinkTicks;
    ...
    //process movement
    if(state != STATE_IDLE && !(state == STATE_IDLESUMMON && hasLostMaster) && this->getName() != "Training Monk")
    {
        if(state == STATE_TARGETNOTREACHABLE)
        {
            ...
            {
                int32_t dx = newMovePos.x - this->pos.x;
                int32_t dy = newMovePos.y - this->pos.y;
...
int32_t Monster::getTargetDistance()
{
    return mType->targetDistance;
}

int32_t Monster::getCurrentDistanceToTarget(const Position& target)
{
...
void Monster::updateLookDirection()
{
    ...
    int32_t deltax = targetPos.x - this->pos.x;
    int32_t deltay = targetPos.y - this->pos.y;
...
bool Monster::calcMovePosition()
{
    ...
    else
    {
        ...
        if(!foundDistPath)
        {
            int32_t currentDist = getCurrentDistanceToTarget(targetPos);
...
bool Monster::getDistancePosition(const Position &target, const int32_t& maxTryDist, bool fullPathSearch, Position &dest)
{
    int32_t minTryDist = 0;
    int32_t currentDistance = getCurrentDistanceToTarget(target);
    ...
    int32_t tryWalkDist = 0;
    int32_t minWalkDist = 0;

    int32_t xmindelta = ((fullPathSearch || (pos.x - target.x) <= 0) ? maxTryDist : 0);
    int32_t xmaxdelta = ((fullPathSearch || (pos.x - target.x) >= 0) ? maxTryDist : 0);
    int32_t ymindelta = ((fullPathSearch || (pos.y - target.y) <= 0) ? maxTryDist : 0);
    int32_t ymaxdelta = ((fullPathSearch || (pos.y - target.y) >= 0) ? maxTryDist : 0);

    for (int32_t y = target.y - ymindelta; y <= target.y + ymaxdelta; ++y)
    {
        for (int32_t x = target.x - xmindelta; x <= target.x + xmaxdelta; ++x)
        {
    ...
    size_t index = random_range(0, (int32_t)positionList.size() - 1);
...
bool Monster::getCloseCombatPosition(const Position &target, Position &dest)
{
    ...
    Position tmpPos;
    int32_t prevdist = 0;
    for(int32_t y = target.y - 1; y <= target.y + 1; ++y)
    {
        for(int32_t x = target.x - 1; x <= target.x + 1; ++x)
        {
            if((target.x == x && target.y == y))
                continue;

            int32_t dist = std::max(std::abs(pos.x - x), std::abs(pos.y - y));
...
void Monster::onThingTransform(const Thing* thing,int32_t stackpos)
{
    if(!thing)
        return;
                ...
                for(it = mType->summonSpells.begin(); it != mType->summonSpells.end(); ++it)
                {
                    if(getRandom() < (*it).summonChance)
                    {
                        ...
                        if(summon)
                        {
                            ...
                            summonPos.x += random_range(-1,1);
                            summonPos.y += random_range(-1,1);
...
void Monster::doMoveTo(int32_t dx, int32_t dy)
{
...
bool Monster::monsterMoveItem(Item* item, int32_t radius)
{
    ...
    for(int32_t i = 0; i < 4*radius; i++)
    {
        ...
        if(game->map->canThrowObjectTo(item->pos, tryPos, BLOCK_SOLID) == RET_NOERROR)
        {
            ...
            int32_t oldstackpos = fromTile->getThingStackPos(item);
...
bool Monster::canMoveTo(uint16_t x, uint16_t y, unsigned char z)
{
