...
bool IOMapOTBM::loadMap(Map* map, std::string identifier)
{
    ...
    if(!propStream.getStruct(root_header))
    {
        return false;
    }
    ...
    while(propStream.getByte(attribute))
    {
        ...
        case OTBM_ATTR_DESCRIPTION:
            if(!propStream.getString(map_description))
            {
                return false;
            }
            break;
        ...
        if(type == OTBM_TILE_AREA)
        {
            ...
            while(tile_node != NO_NODE)
            {
                ...
                if(type == OTBM_TILE)
                {
                    ...
                    if(!propStream.getStruct(tile_coord))
                    {
                        return false;
                    }
                    uint16_t px, py, pz;
...
Item* IOMapOTBM::unserializaItemNode(FileLoader* f, NODE node)
{
    ...
    if(iType.id != 0)
    {
        ...
        while(propStream.getByte(attr_type))
        {
            ...
            case OTBM_ATTR_ACTION_ID:
                if(!propStream.getShort(tmp_short))
                {
                    delete item;
                    return NULL;
                }
            ...
            case OTBM_ATTR_TELE_DEST:
                ...
                if(!propStream.getStruct(tele_dest))
                {
                    delete item;
                    return NULL;
                }
