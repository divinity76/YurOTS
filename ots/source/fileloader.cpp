...
{
    uint32_t size;
    if(const unsigned char* a = getProps(node, size))
    {
...
int32_t FileLoader::setProps(void* data, uint32_t size)
{
    //data
...
int32_t FileLoader::loadCacheBlock(uint32_t pos)
{
    ...
    if(loading_cache == -1)
    {
        for(i = 0; i < CACHE_BLOCKS; i++)
        {
            if((int32_t)(std::abs((int32_t)m_cached_data[i].base - base_pos)) <= (int32_t)(2 * m_cache_size))
                continue;
